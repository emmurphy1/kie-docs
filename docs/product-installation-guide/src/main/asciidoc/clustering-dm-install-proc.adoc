...
Complete the steps in this section to install {CENTRAL}, Configure

ou can use the embedded JMS broker in {EAP}. To do this, install {EAP} and edit the `domain.xml` file as described in <<clustering-dm-install-proc_{context}>>. on each node of the cluster.
