...
. Download *{PRODUCT} 7.0.0 Add Ons* (the `{PRODUCT_INIT}-{PRODUCT_VERSION_LONG}-add-ons.zip` file).
. Unzip the `{PRODUCT_INIT}-{PRODUCT_VERSION_LONG}-add-ons.zip` file. The `rhpam-7.0-controller-ee7.zip` file is in the unzipped directory.
