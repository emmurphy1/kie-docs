...
= Configuring {EAP} 7.1 host.xml files for a clustered environment

After you have installed {EAP} and configured the `domain.xml` file for a clustered environment, you must edit the `host.xml` file on each node of the cluster.

.Prerequisite
{EAP} 7.1 is installed on each node of the cluster.

.Procedure
. On each node of the cluster, open the `_EAP_HOME_/domain/configuration/host.xml` file in a text editor.
...
<system-properties>
 ...
 <property name="org.uberfire.nio.git.dir" value="/tmp/jbpm/niogit"/>
