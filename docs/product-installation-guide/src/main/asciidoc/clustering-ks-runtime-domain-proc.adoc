...
..  Rename `kie-server.zip` file to `kie-server.war`. This is the file that you will use to deploy {CENTRAL} on the cluster nodes.
...
---- 
.. Add the following properties to the `<system-properties>` tag:
+
