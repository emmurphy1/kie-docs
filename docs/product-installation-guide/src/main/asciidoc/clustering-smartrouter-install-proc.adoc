...
. Extract the downloaded `{PRODUCT_FILE}-add-ons.zip` file to a temporary directory. The `rhpam-7.0-smart-router.jar` file is in the extracted `{PRODUCT_FILE}-add-ons` directory.
. Copy the `rhpam-7.0-smart-router.jar` file to location where you will run the file.
...
-Dorg.kie.server.controller.templatefile.watcher.enabled=<TRUE>
