...
. Edit the `data-stores` property and the `timer-service thread-pool-name` above it. The `datasource-jndi-name` is the JNDI name of the database specified at the beginning of this procedure. You can enter any name for the value of the `partition` property. Replace `"default-file-store"` with `"ejb_timer-ds"`· Set the value of `refresh-interval` to specify how often the EJB timer connects to the database to synchronize and load tasks to be processed:
+
[source,xml]
----
