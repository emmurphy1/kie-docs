...
*Properties Required {KIE_SERVER}*
...
ifdef::DM[]
. Click *New* to create a new group. Create the following groups: `admin`, `analyst`, and `kie-server`. Also create the REST API groups if you will use API. 

. Click the *Users* tab and click *New* to create a new user. Provide this new user with a name (for example `decision-server-admin`) and a password. Click *OK* to save.
