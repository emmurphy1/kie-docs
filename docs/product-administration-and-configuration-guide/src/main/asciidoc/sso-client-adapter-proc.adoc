...
To set up the RH-SSO client adapter for {EAP}, install the RH-SSO adapter for {EAP} then configure {PRODUCT} and the RH-SSO client adapter.

.Prerequisites
* RH-SSO is installed as described in <<sso-realm-proc>>.
* {PRODUCT} is installed in a {EAP} 7.1 server, as described in _INSTALLING AND CONFIGURING RED HAT PROCESS AUTOMATION MANAGER ON EAP 7.1_.. 
