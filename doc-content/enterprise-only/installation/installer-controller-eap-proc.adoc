...
. Enter the controller URL for {CENTRAL} and configure additional properties for {KIE_SERVER}. {CONTROLLER} URL should look like this:
* HOST:PORT/business-central/rest/controller

  Replace HOST:PORT with an address of {CENTRAL} located on second machine.

. Complete the installation.
. To verify the setup is successful. Access *Execution Servers* page in {CENTRAL}.
  You should see the connected {KIE_SERVER} under *REMOTE SERVERS*
